...
class TimeUnixLeap(TimeFromEpoch):
    ...
    From https://docs.astropy.org/en/stable/time/index.html#writing-a-custom-format
    """
...
def NMEA2SNR(locdir, fname, snrfile, csnr, dec, year, doy, llh, sp3, compress):
    ...
    if sp3 and (not foundcoords):
        ...
        if fexists:
            ...
            print(
                "This code requires lat/lon/ht inputs. Provide on the command line or"
            )
    ...
    if station == "argt":
        gt.decipher_argt(
            station, "jul01.txt", idec, snrfile, orbfile, recv, csnr, year, month, day
        )
        if os.path.isfile(snrfile):
            print("File made, ignoring the rest of the code")
            return
        else:
            print("Translation was unsuccessful")
            return

    # calculate leap seconds
    current_epoch = Time(str(year) + "-" + f"{month:02}" + "-" + f"{day:02}")
    leapsec = current_epoch.unix_leap - current_epoch.unix - 19  # TAI to GPS time
    ...
    if os.path.exists(locdir + fname):
        ...
        t, prn, az, elv, snr, freq = read_nmea(fname, leapsec)  # read nmea files
    ...
    for i_prn in prn_unique:
        ...
        time = t[prn == i_prn]
        angle = elv[prn == i_prn]
        azimuth = az[prn == i_prn]
        frequency = freq[prn == i_prn]
        Snr = snr[prn == i_prn]
        Prn = prn[prn == i_prn]

        angle_fixed, azim_fixed = fix_angle_azimuth(
            time, angle, azimuth
        )  # fix the angles
        ...
        T.extend(time)
        AZ.extend(azim_fixed)
        ELV.extend(angle_fixed)
        SNR.extend(Snr)
        PRN.extend(Prn)
        FREQ.extend(frequency)
        ...
        for i in range(0, len(timetags)):
            if (timetags[i] % idec) == 0:
                ...
                for ij in range(0, len(unique_sats_epoch)):
                    ...
                    fout.write(
                        "{0:8.0f} {1:3.0f} {2:6.2f} {3:6.2f} {4:6.2f} \n".format(
                            timetags[i], sat, s1, s2, s5
                        )
                    )
        ...
        for i in range(0, len(T)):
            if (int(t[i]) % idec) == 0:
                ...
                if (sat > 100) & (sat < 200):
                    sat = sat - 64
                fout.write(
                    "{0:8.0f} {1:3.0f} {2:6.2f} {3:s} \n".format(
                        T[i], sat, SNR[i], freq[i]
                    )
                )
        ...
        for i in range(len(T)):
            if (float(ELV[i]) >= emin) and (float(ELV[i]) <= emax):
                ...
                # BEIDOU
                    ...
                    p = float(
                        PRN[i]
                    )  # -100 ###Not sure how this could interact with future processing steps...
...
def read_nmea(fname, leapsec):
    ...
    leapsec: int
        number of leapseconds to add to NMEA UTC output to convert to GPS time
    ...
    for i, line in enumerate(lines):
        ...
        if (
            "$" in line[1:-1]
        ):  # Skip line if misplaced $ via https://github.com/purnelldj/gnssr_lowcost
            continue
        if not line.startswith("$G"):  # Skip line if improper start
            continue
        if (
            len(line.split("*")) > 2
        ):  # Additional error checking for misplaced * or combined lines
            continue
        ...
        if "RMC" in line:  # Get timing info
            ...
            try:
                curdt = datetime.datetime(
                    int(row[9][4:6]) + 2000, int(row[9][2:4]), int(row[9][0:2])
                )  # Current date
            except:
                pass
        ...
        if (
            not curdt
        ):  # Skip forward until the first 'RMC' instance which gives a proper date to store
            continue
        ...
        if "GGA" in line:  # read GPGGA sentence: Global Positioning System Fix Data
            ...
            # add leapseconds to convert to GPS time
            t_sec += leapsec
        ...
        # elevations, azimuths, and SNR values.
            ...
            try:
                ...
                if len(sent) in [
                    ...
                    13,
                    9,
                ]:  # Some chips spit out an extra piece of info at the end
                    ...
                    sig_id = sent[-1].split("*")[
                        0
                    ]  # Store that signal ID to check frequency
                    sent = sent[
                        :-1
                    ]  # Remove the last line for subsequent SNR data gatheriing
                    if "GPGSV" in line:
                    ...
                    if "GAGSV" in line:
                        if sig_id in ["6", "7"]:
                            ...
                            sig_id = 6
                    if (
                        "GBGSV" in line or "BDGSV" in line
                    ):  # Beidou has two possible calls in NMEA (and weird frequency codes)...
                ...
                for inds in [
                    ...
                    12,
                    16,
                ]:  # Check for a max of four satellites in view per line
                    try:
                        sat_prn = (
                            int(sent[inds]) + prn_offset
                        )  # field 4,8,12,16 :  SV PRN number
                        elev = sent[
                            inds + 1
                        ]  # field 5,9,13,17 :  Elevation in degrees, 90 maximum
                        azi = sent[inds + 2]  # field 6,10,14,18:  Azimuth in degrees
                        snr_ = sent[
                            inds + 3
                        ]  # field 7,11,15,19:  SNR, 00-99 dB (null when not tracking)
...
def fix_angle_azimuth(time, angle, azimuth):
    ...
    # delet nans
    time = time[~np.isnan(angle)]
    azimuth = azimuth[~np.isnan(angle)]
    angle = angle[~np.isnan(angle)]
    # delet nans
    time = time[~np.isnan(azimuth)]
    angle = angle[~np.isnan(azimuth)]
    azimuth = azimuth[~np.isnan(azimuth)]
    # delet nans
    angle = angle[~np.isnan(time)]
    azimuth = azimuth[~np.isnan(time)]
...
def azimuth_mean(azim1, azim2):
    ...
    if np.all(azim1 >= 0) and np.all(azim2 >= 0):
        ...
        x1 = np.sin(azim1 * np.pi / 180)
        x2 = np.sin(azim2 * np.pi / 180)
        y1 = np.cos(azim1 * np.pi / 180)
        y2 = np.cos(azim2 * np.pi / 180)
        x = ((x1 + x2) / 2.0)[0]
        y = ((y1 + y2) / 2.0)[0]
...
def run_nmea2snr(
    station, year_list, doy_list, isnr, overwrite, dec, llh, sp3, compress
):
    """
    ...
    for yr in year_list:
        ...
        for dy in doy_list:
            ...
            if (not illegal_day) and (not snre):
                r = (
                    station + cdoy + "0." + cyy + ".A"
                )  # nmea file name example:  WESL2120.21.A
                if (
                    os.path.exists(locdir + r)
                    or os.path.exists(locdir + r + ".gz")
                    or os.path.exists(locdir + r + ".Z")
                    or (station == "argt")
                ):
